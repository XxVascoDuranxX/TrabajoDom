...
const estados = ["todo", "in_progress", "done"];
    ...
    function añadirMision() {
        const inputs = document.querySelectorAll("#Seccion3 input");

        const nombre = inputs[0].value.trim();
        const descripcion = inputs[1].value.trim();
        const piloto = document.getElementById("selectPiloto").value;
        const dificultad = document.getElementById("selectDificultad").value;
    ...
    //funcion tipo la de render de arriba
    function pintarKanban(lista = misiones) {
        ...
        lista.forEach(m => {
            ...
            card.innerHTML = `
                ...
                <button onclick="mover(${m.id}, -1)">⬅</button>
                <button onclick="mover(${m.id}, 1)">➡</button>
                <button onclick="eliminar(${m.id})">X</button>
            `;

            const columna = document.querySelector(`[data-estado="${m.estado}"]`);
    ...
    // botón añadir mision
    const btn = document.getElementById("btnAñadirMision");
    if (btn) {
        btn.addEventListener("click", añadirMision);
    }
